...
## 当前原型收尾口径（2026-09-14 用户决定）

保留 c752807 及以前成果；后审查三个代码缺口已按 [阶段七](stage-seven.md) 修复并验证（实现提交 `e4e5c39`，见 [原型收尾证据](evidence/stage-seven-prototype.md)），原型退出条件满足，本轮开发结束。网站访问、代理、许可、大附件和历史 partial 数据按 [人工补齐清单](manual-follow-up.md) 交接，不再作为开发前置。以下旧“完成/下一步”均限原阶段和证据范围，不触发旧任务重跑。
...
## 2026-09-14 增量（阶段七，不重写上文 2026-09-11 基线）

- 原型收尾（同一日后续）：P7-01—P7-03 三项后审查缺口修复，实现提交 `e4e5c39`：附件/正文恢复
  隔离（失败账新增可选 `discovery_method`，续作关联需 URL 证明，身份不足转人工）、`crawl resolve`
  按同一身份协调队列（其他开放阶段不误关、`manual_review` 不自动补抓、写入失败可重复恢复）、
  身份参数区分未指定与显式空值。定向 4/8/2 项、全量 523 passed、扩展闭环最终 `check ok=True`，
  见 [原型收尾证据](evidence/stage-seven-prototype.md) 与
  [闭环日志](evidence/logs/stage-seven-prototype-loop.txt)。
- 新增错误处置命令面：`crawl failures`（默认只列未关闭，`--all` 输出历史与处置行，`--summary`
  有界摘要，`--scope-start-date`/`--doc-id` 身份定位，行内 `next=`/`raw=`）与 `crawl resolve`
  （按显式身份追加处置行，身份没写全以退出码 2 拒绝）。实现提交 `cc7a380`（S7-01 四项修复：
  续作完整性、非 HTML 原件留存、身份统一、正文顺序）、`51b66dc`（S7-02）与收口提交
  （规划对齐与交付收口：阶段身份对账、不支持续接转人工、失败账写入失败退出码 2；提交号以 `git log` 为准），见
  [阶段七交付证据](evidence/stage-seven-delivery.md)。
- 新增开发工具 `tools/closed_loop_fixture.py`（只连 127.0.0.1 夹具站点的有界闭环自检）与证据日志
  `evidence/logs/stage-seven-*.txt`（复现、定向、全量、闭环、只读评估、文档一致性）。
- 候选回归 514 passed（阶段六 492 + 本轮新增 23 − 重写 1）；正式数据根只读评估 `check ok=true`、
  追溯 1.0、对账通过（`open_failures=4`、`open_failures_by_stage={fetch: 4}`；39 篇 `partial` 文档
  属来源窗口的数据完整性）。
- 第 1 节源码提交与第 2 节 `tools/` 规模为 2026-09-11 基线记录；最新提交号以 `git log` 为准，
  `tools/` 现有 7 个脚本。
