...
- DEV-013：stage-seven.md 的 P7-01—P7-03 三个后审查代码缺口已修复并验证（实现提交 `e4e5c39`，证据见 specs/001-public-knowledge-collection/evidence/stage-seven-prototype.md），原型退出条件已满足，阶段七采集原型已交付，本轮开发结束并转为实际问题驱动维护。保留已有成果，不重做旧 S7-01/02/03；不主动调查代理/网站许可或补抓缺失数据，外部错误按 manual-follow-up.md 交由人工处理。不得以覆盖所有异常、消灭全部 issue 或清空失败账作为完成条件。静默丢原件、错误成功、跨对象误关闭及追溯破坏必须修复或可靠隔离；其他可控异常允许有限重试、人工处置与明确披露后交付。软件交付与特定来源窗口的数据完整性分别报告，不自动开启新阶段或部署。
