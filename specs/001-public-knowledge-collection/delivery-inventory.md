...
- 新增错误处置命令面：`crawl failures`（默认只列未关闭，`--all` 输出历史与处置行）与 `crawl resolve`
  （按显式身份追加处置行，身份没写全以退出码 2 拒绝）。实现提交 `b779562`（S7-01 四项修复：
  续作完整性、非 HTML 原件留存、身份统一、正文顺序）与 `62d3648`（S7-02），见
  [阶段七交付证据](evidence/stage-seven-delivery.md)。
- 新增开发工具 `tools/closed_loop_fixture.py`（只连 127.0.0.1 夹具站点的有界闭环自检）与证据日志
  `evidence/logs/stage-seven-*.txt`（复现、定向、全量、闭环、只读评估）。
- 候选回归 507 passed（阶段六 492 + 本轮新增 15）；正式数据根只读评估 `check ok=true`、追溯 1.0、
  对账通过（4 项未关闭失败只计数；39 篇 `partial` 文档属来源窗口的数据完整性）。
